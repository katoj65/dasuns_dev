...
<li><span>DOB</span><span>{{ user.dob.split('-').reverse().join('-') }} </span></li>
...
<div class="card-header d-sm-inline  p-3 bg-danger-dim mb-3" v-if="response.user_data.pssp_attributes.interview_failure.length>0">
<div v-for="m in response.user_data.pssp_attributes.interview_failure" :key="m.id">
<div><strong class="text-danger">
<em class="icon ni ni-cross-circle-fill text-danger mr-1"></em>
{{ m.title }}
</strong>
...
<td colspan="8" style="border:none;padding:10px;padding-left:10px;">
<em class="icon ni ni-dot-sq"></em> {{ s.name }}
</td>
</tr>
<tr>
...
<tbody v-else><tr><td class="text-warning">Missing</td></tr></tbody>
</table>
</div><!-- data-item -->
<div class="data-item p-0 border-0">
<table class="table mt-3">
...
<tbody v-else>
<tr>
<td class="text-warning">Missing</td></tr>
</tbody>
</table>
</div>
</div>
